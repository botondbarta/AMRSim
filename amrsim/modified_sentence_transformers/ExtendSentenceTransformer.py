...
class ExtendSentenceTransformer(nn.Sequential):
    ...
    def __init__(self, model_name_or_path: Optional[str] = None,
        ...
        if cache_folder is None:
            ...
            if cache_folder is None:
                ...
                cache_folder = os.path.join(torch_cache_home, 'modified_sentence_transformers')
    ...
    def save(self, path: str, model_name: Optional[str] = None, create_model_card: bool = True,
        ...
        if '__version__' not in self._model_config:
            self._model_config['__version__'] = {
                'modified_sentence_transformers': __version__,
    ...
    def fit(self,
        ...
        :param evaluator: An evaluator (modified_sentence_transformers.evaluation) evaluates the model performance during training on held-out dev data. It is used to determine the best model that is saved to disc.
    ...
    def _load_auto_model(self, model_name_or_path, adapter_size, gnn, gnn_layer):
        ...
        """
        logger.warning(
            "No entrypoints model found with name {}. Creating a new one with MEAN pooling.".format(
                model_name_or_path))
    ...
    def _load_sbert_model(self, model_path):
        """
        Loads a full entrypoints model
        """
        ...
        if os.path.exists(config_sentence_transformers_json_path):
            ...
            if '__version__' in self._model_config and 'modified_sentence_transformers' in self._model_config['__version__'] and \
                    self._model_config['__version__']['modified_sentence_transformers'] > __version__:
                logger.warning(
